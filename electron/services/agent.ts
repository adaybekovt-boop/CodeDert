...
export const agent = {
  async chat(params: AgentParams, win: BrowserWindow): Promise<{ ok: boolean; error?: string }> {
    ...
    // The primer is built dynamically so the model is ONLY told about tools
    // that will actually fire in this context — advertising run_command while
    // the terminal is disabled, or mcp_call with no servers, just wastes steps
    // and confuses weaker models. (Chekhov's rule: every listed tool must work.)
    const enabledMcpServers = (settings.mcp?.servers || []).filter((s) => s.enabled).length;
    const toolPrimer = buildToolProtocolPrompt({
      allowTerminal: settings.agent.allowTerminal,
      hasMcp: enabledMcpServers > 0,
      // read_recipe only makes sense inside the cdesign flow; detect it from the
      // caller's system prompt instead of threading a new flag everywhere.
      isCdesign: /cdesign/i.test(system || ''),
    });
    const systemFinal = [system, toolPrimer].filter(Boolean).join('\n\n');
...
export interface ToolPromptContext {
  /** run_command tool is offered only when the terminal is enabled. */
  allowTerminal: boolean;
  /** mcp_list_tools / mcp_call are offered only when ≥1 MCP server is enabled. */
  hasMcp: boolean;
  /** read_recipe is offered only inside the cdesign flow. */
  isCdesign: boolean;
}

/**
 * Build the model-facing tool protocol primer. Only tools that will actually
 * execute in the current context are listed — a tool described to the model
 * but guaranteed to fail (terminal off, no MCP servers) wastes turns and
 * derails weaker models. Tools are numbered dynamically so the list always
 * reads as a clean 1..N regardless of which optional tools are present.
 */
export function buildToolProtocolPrompt(ctx: ToolPromptContext): string {
  const header = `У тебя есть РУКИ — инструменты для работы с файлами проекта пользователя.
...
ВНИМАНИЕ: ниже перечислены ТОЛЬКО реально доступные сейчас инструменты. Не вызывай ничего, чего нет в этом списке — такой вызов вернёт ошибку.

Доступные инструменты:`;

  // Core tools — always available.
  const core: string[] = [
    `read_file — прочитать файл. Для больших файлов читай по частям: необязательные <offset> (номер первой строки, с 1) и <limit> (количество строк). Ответ содержит total_lines; если файл показан не полностью — продолжай с указанного offset, пока не увидишь всё нужное.
<tool name="read_file">
...
<limit>400</limit>
    ...
    `search — найти код по проекту (регистронезависимый regex или подстрока). Возвращает строки вида path:line: текст. Используй ЭТО, чтобы находить функции/символы/места использования.
<tool name="search">
<query>function handleSubmit</query>
</tool>
(необязательный <path>src</path> ограничивает поиск папкой.)`,

    `list_dir — список содержимого папки (без path = корень проекта).
<tool name="list_dir">
<path>src/components</path>
</tool>`,

    `edit_file — ТОЧЕЧНАЯ замена. old_string должен встречаться в файле РОВНО ОДИН РАЗ (включая отступы и пробелы). Если фрагмент не уникален — расширь old_string соседними строками для уникальности. Только при явной необходимости поставь <replace_all>true</replace_all> для замены всех вхождений.
<tool name="edit_file">
<path>src/foo.ts</path>
...
вторая строка</new_string>
    ...
    `create_file — создать новый файл (или перезаписать существующий — но для существующих предпочитай edit_file).
...
<content>export const x = 1;
    ...
    `delete_file — удалить файл (используй с осторожностью, только когда пользователь явно попросил).
...
<path>src/old.ts</path>
  ...
  if (ctx.isCdesign) {
    optional.push(
      `read_recipe — прочитать один из встроенных рецептов cdesign (полезно только в режиме CDESIGN). Имена совпадают с файлами в references/recipes/ без расширения .md: lenis-gsap-sync, pinned-scrub, r3f-photo, split-reveal, scroll-film, liquid-glass, multi-layer-parallax, canvas-scrub, easing и т.д.
...
<name>pinned-scrub</name>
  ...
  if (ctx.allowTerminal) {
    optional.push(
      `run_command — выполнить shell-команду внутри workspace пользователя. Оболочка: PowerShell на Windows, /bin/sh на Linux/macOS. Захватываются stdout + stderr + exit code. Используй для: запуска тестов (npm test), установки зависимостей (npm install), git (git status, git diff, git log), линтеров/форматтеров. По умолчанию каждая команда требует явного подтверждения пользователя (появляется диалог). НИКОГДА не выполняй разрушительные операции без явной просьбы (rm -rf, format, shutdown — они ЖЁСТКО ЗАБЛОКИРОВАНЫ в любом случае).
<tool name="run_command">
<command>npm test</command>
<cwd>packages/web</cwd>
<timeout_ms>120000</timeout_ms>
</tool>
- <cwd> необязателен (по умолчанию корень workspace). Должен резолвиться ВНУТРИ workspace.
- <timeout_ms> необязателен (по умолчанию настроенный таймаут, максимум 30 минут).`
    );
  }

  if (ctx.hasMcp) {
    optional.push(
      `mcp_list_tools — список внешних MCP-инструментов, подключённых пользователем (файловые системы, базы данных, браузеры, API и т.д.). Вызови один раз, если задача может требовать внешних возможностей.
<tool name="mcp_list_tools">
</tool>`,
      `mcp_call — вызвать MCP-инструмент. <arguments> — JSON-объект по schema из mcp_list_tools.
<tool name="mcp_call">
...
<arguments>{"path": "/tmp/data.json"}</arguments>
  ...
  // ask is always available — it only shows a prompt, never touches the FS.
  const ask = `ask — задать вопрос человеку и ДОЖДАТЬСЯ ответа. Используй ТОЛЬКО когда реально заблокирован: не хватает решения, которое может принять лишь пользователь (выбор варианта, подтверждение рискованного действия, недостающий факт, который нельзя получить из файлов). Не спрашивай о том, что можно узнать через read_file/search. Один конкретный вопрос за раз.
<tool name="ask">
...
Система покажет вопрос пользователю и вернёт его ответ в <tool_result>. Если пользователь пропустил — прими решение сам.`;
  ...
  const allTools = [...core, ...optional, ask];
  const numbered = allTools.map((t, i) => `${i + 1}) ${t}`).join('\n\n');
...
/**
 * Backwards-compatible full primer (every optional tool present). Kept for any
 * caller/test that imports the constant directly; the live agent loop uses
 * buildToolProtocolPrompt() with the real per-request context instead.
 */
export const TOOL_PROTOCOL_PROMPT = buildToolProtocolPrompt({
  allowTerminal: true,
  hasMcp: true,
  isCdesign: true,
});
