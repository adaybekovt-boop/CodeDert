 ...
 */
function findCompletedToolCall(buffer: string): ToolCall | null {
 ...
 */
function collapseOldToolResults(convo: AgentMessage[], keepLastN = 2): void {
...
export const agent = {
  async chat(params: AgentParams, win: BrowserWindow): Promise<{ ok: boolean; error?: string }> {
    ...
    const lock = ownsLock
      ? aiMutex.acquire('agent', requestId, () => {
 ...
 * We keep a small trailing buffer (the unsafe zone) to avoid leaking `<tool` to the UI.
 */
function findSafeTextBoundary(buffer: string, from: number): number {
  // If there's an open <tool tag already, hide from its start.
  const open = buffer.indexOf('<tool', from);
